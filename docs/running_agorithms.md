...
* **DROID-SLAM** runs in the `droidenv` conda env. For long sequences (HortiMulti, Rosario) full frames are used (no stride). For the initial Rosario seq1 run `stride=2` was used; trajectory length will be ~half the input. Current standard is stride=1 (full frames).
